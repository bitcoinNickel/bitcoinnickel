...
###   bitcoinnickelcore.org and bitcoinnickel.org and compares them.
...
WORKINGDIR="/tmp/bitcoinnickel_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinnickelcore.org"
HOST2="https://bitcoinnickel.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinnickel-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinnickel-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinnickel.org failed to provide signature file, but bitcoinnickelcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinnickel.org and bitcoinnickelcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinNickel Core binary release signing key installed?"
   fi
