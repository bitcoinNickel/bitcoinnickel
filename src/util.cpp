...
const char * const BITCOINNICKEL_CONF_FILENAME = "bitcoinnickel.conf";
const char * const BITCOINNICKEL_PID_FILENAME = "bitcoinnickeld.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitcoinnickeld -foo=bar
...
#else
    const char* pszModule = "bitcoinnickel";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinNickel
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinNickel
    // Mac: ~/Library/Application Support/BitcoinNickel
    // Unix: ~/.bitcoinnickel
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinNickel";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinNickel";
#else
    // Unix
    return pathRet / ".bitcoinnickel";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINNICKEL_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BITCOINNICKEL_PID_FILENAME)));
}
